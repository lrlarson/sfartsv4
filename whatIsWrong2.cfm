...
WHERE     (Event_Table.Start_Date <= DATEADD(day, #Recordset2__date_ahead#, { fn CURDATE() })) AND (Event_Table.End_Date >= DATEADD(day, 
                      #Recordset2__date_ahead# -1, dbo.getPacificTime())) AND (Event_Table.Performing_Arts_Event = 1) AND (Event_Table.[#Recordset2__num#] = 1) OR
                      (Event_Table.Start_Date <= DATEADD(day, #Recordset2__date_ahead# - 1, dbo.getPacificTime())) AND (Event_Table.End_Date >= DATEADD(day, 
                      #Recordset2__date_ahead# - 1, dbo.getPacificTime())) AND (Event_Table.[#Recordset2__day_string#] = 1) AND (Event_Table.Gallery_Museum_Event = 1) OR
                      (Event_Table.Start_Date <= DATEADD(day, #Recordset2__date_ahead# -1 , dbo.getPacificTime())) AND (Event_Table.End_Date >= DATEADD(day, 
                      #Recordset2__date_ahead# - 1, dbo.getPacificTime())) AND (Event_Table.Performing_Arts_Event = 1) AND (Event_Table.[#Recordset2__num#] = 1) OR
                      (Event_Table.Start_Date <= DATEADD(day, #Recordset2__date_ahead# - 1, dbo.getPacificTime())) AND (Event_Table.End_Date >= DATEADD(day, 
                      #Recordset2__date_ahead# - 1,dbo.getPacificTime())) AND (Event_Table.[#Recordset2__day_string#] = 1) AND (Event_Table.Gallery_Museum_Event = 1)
